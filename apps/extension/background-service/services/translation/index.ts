...
export class TranslationService implements IService {
    readonly name = 'translation' as const
    ...
    async initialize(): Promise<void> {
        if (this.initialized) {
            Logger.info('[TranslationService] Already initialized, skipping...')
            return
        }

        try {
            Logger.info('[TranslationService] Initializing translation service...')
            
            const configService = ConfigService.getInstance()
            const config = await configService.getTranslationConfig()

            // 初始化各个翻译提供商
            this.translators.set('google', new GoogleTranslator(config))

            this.initialized = true
            Logger.info('[TranslationService] Translation service initialized successfully')
        } catch (error) {
            Logger.error('[TranslationService] Failed to initialize:', error)
            throw error
        }
    }
    ...
    async translateText(text: string, provider?: string): Promise<string> {
        ...
        try {
            ...
            Logger.error(`[TranslationService] Translation error with ${selectedProvider}:`, error)
            // 尝试备用翻译
            return await this.fallbackTranslate(text)
        }
    }
    ...
    async updateConfig(): Promise<void> {
        ...
        Logger.info('[TranslationService] Configuration updated')
    }
    ...
    async cleanup(): Promise<void> {
        ...
        Logger.info('[TranslationService] Cleaned up successfully')
    }
}