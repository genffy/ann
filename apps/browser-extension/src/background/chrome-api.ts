...
export function getChromeAPI(chrome = globalThis.chrome) {
   ...
   */
  function promisify<Args extends any[], Result>(fn: (...args: [...Args, Callback<Result>]) => void): (...args: Args) => Promise<Result> {
  ...
  function promisifyAlt<Args extends any[], Result>(fn: (...args: Args) => Promise<Result>): (...args: Args) => Promise<Result> {
    // @ts-expect-error
    return promisify(fn)
  }
 ...
 */
export async function executeScript({ tabId, frameId, file }: ExecuteScriptOptions, chromeAPI_ = chromeAPI): Promise<unknown> {
