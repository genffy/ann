...
/**
 * Annhub-related state for a specific tab.
 *
...
export type State = {
  /**
   * Whether the user has activated Annhub for this tab. This state
  ...
  /** Details of error that occurred while trying to activate Annhub in this tab. */
  error?: Error

  /**
   * Query indicating the annotation, group etc. to focus when Annhub loads
   * in this tab.
   */
...
/**
 * TabState stores the Annhub-related state for tabs in the current browser
 * session.
 */
export class TabState {
  ...
  /**
   * Current Annhub-related state for each tab.
   *
  ...
  /**
   * Mark a tab as having Annhub loaded in it.
   */
  ...
  /**
   * Mark a tab as not having Annhub loaded.
   */
  ...
  /**
   * Mark a tab as having encountered an error when trying to load Annhub into it.
   */
  ...
  /**
   * Remove Annhub-related state about a given tab.
   */
  ...
  /**
   * Return the current Annhub-related state for a tab.
   */
  ...
  /**
   * Return `true` if Annhub is loaded into a given tab.
   */
  ...
  /**
   * Return `true` if Annhub is not loaded in a given tab.
   */
  ...
  /**
   * Return `true` if an error occurred while trying to load Annhub into a given tab.
   */
  ...
  async updateAnnotationCount(tabId: number, tabUrl: string) {
    ...
    const wait = Math.min(pendingRequest?.waitMs ?? INITIAL_WAIT_MS, MAX_WAIT_MS)
